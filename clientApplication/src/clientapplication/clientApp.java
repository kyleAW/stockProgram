...
public class clientApp extends javax.swing.JFrame {
    ...
    public clientApp() {
        ...
        startUp();                                                              //the first thing i want it to do as it opens is run startup. it will then get the latest prices it can and populate the table. 
    }
    ...
    private void initComponents() {
        ...
        List<String> currencyList = getCurrencyCodes();                         //take the list generated by getcurrencycodes
        String[] currArray = new String[currencyList.size()];           //create an array the size of the list
        currArray = currencyList.toArray(currArray);                    //convert the list to the array
        ...
        //create a new combo box using the array list to populate the options and using option 49 as default (which is GBP)
    ...
    private void startUp(){                                                     //this is the operation from the update prices button, but as it runs at init its a seperate function that the button then calls
        ...
        List<StockType> collectionStock = allStocks();                          //uses the list from allstocks

        Iterator itr = collectionStock.iterator();                              //itterates through the allstocks
        StockType nextStock;    

        while (itr.hasNext()) {                                                 //while theres a next item in the list
            nextStock = (StockType) itr.next();                     
            List<String> name = new ArrayList();                                //list for stock names
            name.add(nextStock.getName());

            List<String> code = new ArrayList();                                //list for stock codes
            code.add(nextStock.getCode());

            List<Integer> amount = new ArrayList();                             //list for amounts
            amount.add(nextStock.getShareNo());

            List<XMLGregorianCalendar> date = new ArrayList();                  //list for dates
            XMLGregorianCalendar dateInfo = nextStock.getStockPrice().getDate();
            date.add(dateInfo);

            List<String> currrency = new ArrayList();                           //list for currency displayed
            String currencyInfo = arg1;                                         //for the currency it displays what ever is in arg1
            currrency.add(currencyInfo);

            List<Double> price = new ArrayList();                               //list for stock prices
            Double baseCost = nextStock.getStockPrice().getSharePrice();        //it gets the shareprice
            Double convPrice = baseCost * convResult;                           //converts it by the conversion result
            Double formatPrice = Double.parseDouble(df.format(convPrice));      //displays a formatted version of the conversion to reduce the number of decimal 
            ...
            //creates an array list for every item to go into the table. it then populates the table with the items for each column with a .get until there is nothing left in the lists.
        }
    }
    ...
    private void PriceRadioButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_PriceRadioButtonActionPerformed
        DecimalFormat df = new DecimalFormat("0.00");                           //code is generally duplicated through each of the buttons fetching the relevant list for the section
        String arg1;
        try {
            arg1 = splitCurr[0];
    ...
    private void SellBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SellBtnActionPerformed
        try {                                                                   //try to get an amount from the sell amount box
            amountStock = Integer.parseInt(sellAmount.getText());               //convert the entry from sellAmount to an int
            System.out.println("amount :" + amountStock);                       //print the amount to the server

        } catch (NumberFormatException e) {
            System.out.println("problem with number of shares for sale");       //if its not a number it prints problem to the server
            purchasePane.setText("No Shares");                                  //displays outcome to the purchasePane
        }

        try {

            if (selectedStock.trim().isEmpty() || amountStock == 0) {           //checks that the stock is selected from the table
            } else {
                if (shareSell(selectedStock, amountStock) == true) {            //if all outcomes are met and the shareSell returns true
                    purchasePane.setText("Sale Success");                       //sale is made and pruchase pane informs user
                } else {
                    purchasePane.setText("Sale Failed");                        //if it fails it informs the user
                }
            }
        } catch (NullPointerException e) {                                      // if no company is selected it displays this to the user and also sends to the server
            System.out.println("No company selected");          
            purchasePane.setText("No Company");
        }

        int rows = table.getRowCount();                                         //from here the table display is the same as allstock()
        for (int i = rows - 1; i >= 0; i--) {
            table.removeRow(i);
        }
    ...
    private void BuyBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BuyBtnActionPerformed
        try {                                                                   //works the same as the sell button except using the buy function
    ...
    private void stockTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_stockTableMouseClicked
        ...
        int selected = stockTable.getSelectedRow();                             //gets the row of what ever mouse click
        selectedStock = table.getValueAt(selected, 0).toString();               //gets the value at the first column in the selected row
        newsClick(selectedStock.replace(" ","%20"));                            //changes the spaces for web address format so the newsfeed can send it to the api 

    }//GEN-LAST:event_stockTableMouseClicked

    private void newsClick(String comp) {       //create a method to handle the newsfeed when the table is clicked comp is retrieved from the table click action
        ...
        newsReel.close();               //closes the newsfeed api when retrieved what it needs
    ...
    private void CurrencyChoiceActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_CurrencyChoiceActionPerformed
        ...
        DecimalFormat df = new DecimalFormat("0.00");                   //rest is as per other buttons     
    ...
    private void updatePricesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updatePricesActionPerformed
        startUp();                                                        //do the startup function when this button is pressed
